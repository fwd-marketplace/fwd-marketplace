...
// Interfaces for our typesafe state
interface Project {
  id: string;
  title: string;
...
export function PerfilEmpresa() {
  ...
  const initialCandidates: CandidateMatch[] = [
    // Matches for Project 1 (Frontend Senior React)
    {
      id: "c1",
      projectId: "p1",
    ...
    // Matches for Project 2 (UX/UI Designer Sprint)
    {
      id: "c3",
      projectId: "p2",
      name: "Cata Pereira",
    ...
    // Matches for Project 3 (Backend Node.js Architect)
    {
      id: "c5",
      projectId: "p3",
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    ...
    // Left borders cycled for visual interest
    const borderColors: string[] = [
      "border-l-4 border-accent",
      "border-l-4 border-warning",
      "border-l-4 border-secondary",
    ];
    const colorClass: string = borderColors[projects.length % borderColors.length] ?? "border-l-4 border-accent";
  ...
  // --- Computed stats based on current projects & applications ---
  const stats = useMemo(() => {
  ...
  // Filter candidates and applications based on selected project
  ...
  const getStatusText = (status: Application["status"]) => {
    switch (status) {
      ...
      case "accepted":
        return "Aceptado"; // fallback
      case "rejected":
        return "Rechazado"; // fallback
    }
  };
          ...
          {filteredCandidates.length > 0 ? (
            ...
            <div className="flex h-52 flex-col items-center justify-center rounded-2xl border border-border bg-surface p-6 text-center">
              ...
              <p className="font-body text-sm font-semibold text-ink-muted">
                No hay candidatos recomendados para este proyecto en este momento.
              </p>
            </div>
          )}
        </section>
            ...
            <form onSubmit={handleCreateProject} className="space-y-4 pt-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  required
                  placeholder="Ej. E-commerce Mobile Sync"
